...
accessible.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		if fmRunning {
			...
			if forceTearDown {
				warn("%s", "the deploy port forwarding is not running, "+
					"so the remote manager could not be stopped"+noManagerForcedMsg)
				serverHadProblems = true
			} else {
...
func rewriteConfigFiles(configFiles string) []client.FilePair {
	...
	for _, path := range paths {
		if strings.HasPrefix(path, "~/") {
			...
			if err != nil {
				warn("%s", fmt.Sprintf("Could not convert path %s to relative path.", path))
			}
