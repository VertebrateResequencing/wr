...
type ConfigKubernetes struct {
	...
	// by default kubernetes uses kubedns, which will be set by a cluster administrator.
...
func (s *k8s) initialize(config interface{}, logger log15.Logger) error {
	...
	if len(s.config.Image) == 0 {
		s.config.Image = "ubuntu:latest"
	}
	...
	s.libclient.Initialize(kubeClient, s.config.namespace)
