...
- You can now modify non-running incomplete commands from the status web page
  and REST API.
...
- New `wr status --recent <duration>` mode to show successful jobs that finished
  within the last duration, across all report groups.
...
- `wr status` now reports scheduler issues and bad servers (the same problems
  the web UI surfaces) as a footer.
...
- A command that failed for some other reason but happened to exceed its memory
  ...
  RAM; the real failure reason is now given (with the high memory use noted),
  and the expected memory is still raised for later retries when automatic
  resource learning is allowed.
- Bulk adds of many dependent commands no longer miscount persisted live
  commands as duplicates; the missing commands are requeued and counted as newly
  added, and status details show which dependency group they are waiting for.
...
- Fixed a reserved-quota leak in cloud/OpenStack mode when the configured OS
  ...
  manager.
- Updated all dependencies to address potential security vulnerabilities.
...
- New client.FindJobsByRepGroupPrefixAndState() function for eg. finding
  incomplete jobs when interacting with a manager using the go client API.
