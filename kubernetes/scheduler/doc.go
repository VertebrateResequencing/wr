...
deployment controller. They also report status changes of a pod to the
...
channel is retrieved by pod name.  Then when processPod observes a succesfully
...
pod lifecycle means that the same pod will be added to the workqueue multiple
times throughout it's deletion. We only want to send on the channel once, and
the done bool indicates if we have.
...
A nodes' avaliable resources are also monitored, in order to fulfil the
reqCheck() in ../../jobqueue/scheduler/kubernetes.go, a cached total of all
resources avaliable to each node is kept in memory for the controller. A request
